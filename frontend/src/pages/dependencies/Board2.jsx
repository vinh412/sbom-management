...
function Board2({ dependencies, loading }) {
  const {
    token: { colorBgContainer, colorBorder },
  } = theme.useToken();
  let tree = { id: uuidv4(), ref: dependencies[0].ref, dependsOn: [] };
  const [nodes, setNodes] = useState([]);
  const [edges, setEdges] = useState([]);

  const updateRender = () => {
    const treeRender = renderTree(tree, 0, (countLeafNodes(tree) * h) / 2);
    setNodes(treeRender.nodes);
    setEdges(treeRender.edges);
  };
  useEffect(() => {
    tree = { id: uuidv4(), ref: dependencies[0].ref, dependsOn: [] };
    updateRender();
  }, [dependencies]);
  ...
  const handleClick = (id) => {
    const subTree = findTreeById(id, tree);
  ...
  const nodeTypes = useMemo(() => ({ customNode: CustomNode }), [dependencies]);
  return (
    <div
      style={{
        height: "100vh",
