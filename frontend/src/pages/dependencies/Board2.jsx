...
function Board2({dependencies}) {
  ...
  const renderTree = (root, x, y) => {
    ...
    nodes.push({ id: root.id, position: { x, y }, data: { label: root.ref.split("?")[0], hasDependencies}, type: 'customNode' });
    const cd = countLeafNodes(root)*h;
    let current = 0;
    root.dependsOn.forEach((item, idx) => {
      const child = renderTree(item, x + root.ref.length*8 + w, y - cd/2 + countLeafNodes(item)*h/2 + current);
      current += countLeafNodes(item)*h;
      const childNodes = child.nodes;
      const childEdges = child.edges;
      nodes.push(...childNodes);
      edges.push({ id: `${root.id}-${item.id}`, source: root.id, target: item.id });
  ...
  function CustomNode({data, id}) {
    return (
        <div onClick={() => {console.log(id); handleClick(id);}}>
          ...
          {data.hasDependencies && <Handle type="source" position={Position.Right} id="a" />}
        </div>
    );
  }
